...
public class Tester {
	// this class exactly copy pasted from my Main class but in this part ı Will change variable within some different txts so that we can understand that it does work exactly for every other situation
	
	public Tester() {
		// TODO Auto-generated constructor stub
		String tasıyıcı;// this object will used by me to take writings from txt's to normal string  
		File cities = new File("citiesForTester.txt");//instead of writing the url of every txt instead ı created file class and use it to the other class that requires urls
		File vehicles = new File("vehiclesForTester.txt");
		File packages = new File("packagesForTester.txt");
		File missions = new File("missionsForTester.txt");
		City[] city=null;
		...
		try {
			...
			System.out.println("It takes data to City objects from Txt");
			bfReader.close();
		...
		try {
			...
			System.out.println("It takes data to CargoPackage objects from Txt");
			bfReader.close();
		} catch (FileNotFoundException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		...
		try {
				...
				System.out.println("It takes data to Vehcile objects from Txt");
				bfReader.close();
			} catch (FileNotFoundException e1) {
					// TODO Auto-generated catch block
				e1.printStackTrace();
		...
		try {// for the missions ı Created this loop exactly like the others
			...
			System.out.println("It takes data to mission objects from Txt like");
			bfReader.close();
		} catch (FileNotFoundException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		...
		}System.out.println("It collects data to where it should be for the Vehicle");
		...
		}System.out.println("It collects data to where it should be for the cargoPackage");
		...
		if(mission!=null) {
			...
			for(City c :city) {// well ı am aware that if vehicForTeMision is empty it does not work but this would not my fault it can happen only if there is no exactly the same City at the fistDCenter in mission
			...
			}System.out.println("it take everyting to vehicle then delivered to the next one by one");
		...
		for(City a: city) {
			...
			stBfr.append("-------------\r");
			whole.append(stBfr);
		}System.out.println("I turned everythin to a String here");
		...
		File resultFile= new File("resultForTester.txt");
		...
		System.out.println("And here we writed it to a new txt");
	}
